...
## 3.8 EC2本評価（決定的な結果・2026-06-21）★MCTSは不採用
EC2(c7i.4xlarge=16vCPU native)で run_tournament による全フィールド(44体)チャレンジャー評価。同条件(各4戦)で3者比較：

| エージェント | 対フィールド勝率 |
|---|---|
| **Base（mono-Crustleヒューリスティック）** | **68%** |
| 変種C（hybridヒューリスティック） | 62% |
| MCTS det=2（hybrid＋探索） | 55% |
| **MCTS det=8（SOTA級・hybrid＋探索）** | **57%** |

- **結論：MCTSは det=8 に上げても 57%で、ヒューリスティック(Base 68% / 変種C 62%)に明確に劣る。** sim増(det2→8)も+2ptのみ。
- **head-to-head 80%は相手モデル=mirror（相手そのもの）への過剰適合**だった。実フィールドでは相手アーキタイプを当てられず、誤った相手モデルに最適化して劣化。
- 根本原因：①PIMCの相手モデル（決定化＋rollout方策）の質が対フィールド性能を支配し、現状の推定は不十分。②ハイブリッドデッキ自体が mono-Crustle より弱い(62<68)。
- **判断：MCTS路線は現状不採用。** 改善には「相手アーキタイプの高精度推定＋専用rollout方策」という大掛かりな作り込みが要り、費用対効果が見合わない。**最良は依然 mono-Crustle ヒューリスティック(68%)。**
- EC2は評価後に terminate（コスト安全装置A/B＋手動terminateで運用）。
