...
def _save_eval_log(g, tag):
    """1試合のログを Kaggle replay.json 互換形式で LOG_DIR に保存する（対戦と同形式）。"""
    if not LOG_DIR:
        return
    rec = g.get("recorder")
    if rec is None:
    ...
    try:
        ...
        replay["info"]["TeamNames"] = [g.get("player_label", "P1"),
                                       g.get("opponent_label", "P2")]
        replay["info"]["Mode"] = "eval"
        replay["info"]["Tag"] = tag
...
def _run_match_with_deadline(p0, p1, seed, deadline_s, tag=None):
    ...
    if logging_on:
        g = {"obs": obs, "events": [], "log": [], "frames": [], "hand_cache": {},
             ...
             "recorder": ReplayRecorder([p0.get("name", "P1"), p1.get("name", "P2")])}
    result = -1
    ...
    try:
        for _ in range(20000):
            ...
            if logging_on:
                g["recorder"].record(obs, you, picks)   # replay.json 互換で記録
